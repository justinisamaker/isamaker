...
<BlogImage src="/assets/blog/internet-in-a-box/internet-in-a-box.jpg" alt="Internet in a Box running on a Raspberry Pi" caption="Internet in a Box running on a Raspberry Pi" size="large" />
...
<ParagraphWithPicture src="/assets/blog/internet-in-a-box/usb-boot.png" alt="Selecting USB Boot from Raspi-Config" caption="Selecting USB Boot from Raspi-Config">

Once your Pi is booted up, run `sudo raspi-config` or open the config tool via the GUI if you're using a screen. The specific option that we're looking for is under `Advanced Options > Boot Order`. We want our Pi to boot from the USB, so we select "USB Boot".

*Do not reboot your Pi!* Before we reboot, we want to copy our Pi's card over to our external hard drive.
...
<BlogImage src="/assets/blog/internet-in-a-box/nextcloud-sql-error.png" alt="SQL error regarding NextCloud during install" caption="SQL error regarding NextCloud during install" size="large" />
...
<BlogImage src="/assets/blog/internet-in-a-box/manage-content.png" alt="Manage content screen showing available space" caption="Manage content screen showing available space" size="large" />
